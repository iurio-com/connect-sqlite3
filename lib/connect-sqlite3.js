...
module.exports = function(connect) {
    ...
    function dbInit(store) {
        store.db.exec(`${store.options.concurrentDb ? 'PRAGMA journal_mode = wal; ' : ''}CREATE TABLE IF NOT EXISTS ${store.table} (session_id PRIMARY KEY, expires, data)`);
        dbCleanup(store);
    }
    ...
    function SQLiteStore(options) {
        ...
        if (options.db) {
            ...
            if (typeof options.getDB !== 'function') {
                throw new Error('Either db or getDB() option must be set');
            }
            let isInitialized = false;
            Object.defineProperty(this, 'db', {
                get() {
                    let db = this.options.getDB();
                    if (!db?.isOpen) {
                        throw new Error('getDB() did not return an opened database');
                    }
                    if (!isInitialized) {
                        dbInit({ ...this, db });
                        isInitialized = true;
